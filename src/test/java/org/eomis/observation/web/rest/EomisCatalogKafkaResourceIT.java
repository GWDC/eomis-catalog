package org.eomis.observation.web.rest;
...
import org.eomis.observation.IntegrationTest;
import org.eomis.observation.config.EmbeddedKafka;
import org.eomis.observation.config.KafkaSseConsumer;
import org.eomis.observation.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class EomisCatalogKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/eomis-catalog-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/eomis-catalog-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/eomis-catalog-kafka/unregister"));
                return;
            }
        }
